...
def test_upload_mode(orchestrator, mock_managers, db_config, tmp_path, caplog):
    ...
    mock_managers["db"].commit.assert_called()
...
def test_upload_mode_regex(orchestrator, mock_managers, db_config, tmp_path, caplog):
    ...
    # open_file returns a file-like object directly now in our mock
    mock_file = MagicMock()
    ...
    with caplog.at_level("INFO"):
        ...
        mock_managers["db"].update_lobs_batch.assert_called_once()
        batch_data = mock_managers["db"].update_lobs_batch.call_args[0][0]
        actual_ids = {item[1] for item in batch_data}
