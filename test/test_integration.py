...
def test_update_lob_integration(connector):
    # Use ID 3
...
def test_blob_upload_integration(connector, db_config, tmp_path):
    # Use BLOB_DATA table
    ...
    target_id = "1"
    new_binary_content = b"New binary content \x00\x01\x02\x03"

    # We need an orchestrator to test the full flow or use connector directly
    # To follow the plan, let's test the connector's update_lob with binary data
    connector.update_lob(target_id, new_binary_content)
    connector.commit()
    ...
    if hasattr(content, 'read'):
        content = content.read()
    assert content == new_binary_content

def test_xmltype_upload_integration(connector, db_config):
    ...
    target_id = "1"
    new_xml_content = "<root><item>Updated XML</item></root>"

    connector.update_lob(target_id, new_xml_content)
    connector.commit()
    ...
    assert len(results) == 1
    content = results[0][1]
    # XMLType in fetch usually comes back as a string or a LOB that reads to string
    if hasattr(content, 'read'):
        content = content.read()
    assert str(content).strip() == new_xml_content
